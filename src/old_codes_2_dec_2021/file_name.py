...
# df = pd.read_csv("preds_mca.csv")
# print(df[df["values"] != df["preds"]]["values"].__len__())
...
"""
    ***
        code for moving the copy the files from on to another dir
    ***
"""

cap_type = "train"
cap_folder = "rc_captcha"

src_dir = f"../input/all_captcha_types/{cap_folder}/"
target_dir = f"../input/{cap_type}_all_captchas/"
file_names = os.listdir(src_dir)

for file in file_names:
    shutil.copy(os.path.join(src_dir, file), target_dir)


"""
    ***
        code for changes the target variables
    ***
"""
# counts = 0
# for count, filename in enumerate(os.listdir("../input/train_all_captchas/")):
#     file_name = str(filename.split(".")[0])
#     # num_files = file_name.split("#")[0]
#     # try:
#     #     if type(int(num_files)) == int:
#     #         print(filename)
#     # except Exception as e:
#     #     pass

#     if len(file_name) == 6:
#         src = "../input/train_all_captchas/" + str(filename)
#         dst = "../input/train_all_captchas/" + str(file_name) + "#" + ".png"
#         os.rename(src, dst)
#         print(file_name)
    
    #     os.remove(f"../input/train_all_captchas/{filename}")
    # print(count)
